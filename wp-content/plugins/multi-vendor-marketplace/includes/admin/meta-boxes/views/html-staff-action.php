...
<ul class="order_actions submitbox">
	 ...
	 * @since 1.0.0
	 */
	do_action( 'mvr_staff_actions_start', $post->ID );
	?>

	<li class="wide">
		<select name="_status">
			<option value=""><?php esc_html_e( 'Choose a Status', 'multi-vendor-marketplace' ); ?></option>
			<?php foreach ( mvr_get_staff_statuses() as $status_name => $status_label ) { ?>
				<option value="<?php echo esc_attr( $status_name ); ?>" <?php selected( 'mvr-' . $staff_obj->get_status( 'edit' ), $status_name, true ); ?>><?php echo esc_html( $status_label ); ?></option>
			<?php } ?>
		</select>
	</li>
	 ...
	 * @since 1.0.0
	 */
	do_action( 'mvr_staff_actions_end', $post->ID );
	?>
</ul>
<?php
