...
if ( ! class_exists( 'MVR_Email_Staff_Assigned', false ) ) {

	/**
	 * Staff Assigned
	 *
	 * @class MVR_Email_Staff_Assigned
	 * @package class
	 * @extends MVR_Abstract_Email
	 */
	class MVR_Email_Staff_Assigned extends MVR_Abstract_Email {
		...
		public function __construct() {
			$this->id             = 'mvr_email_staff_assigned';
			$this->title          = __( 'New Staff Assigned for Vendor', 'multi-vendor-marketplace' );
			$this->description    = __( 'Staff Assigned emails are sent to Staff when a admin assign Staff to Vendor.', 'multi-vendor-marketplace' );
			$this->template_html  = 'emails/staff-assigned.php';
			$this->template_plain = 'emails/plain/staff-assigned.php';
			...
			add_action( 'mvr_after_create_staff_notification', array( $this, 'trigger' ) );
			add_action( 'mvr_after_assign_staff_notification', array( $this, 'trigger' ) );
			add_action( 'mvr_admin_after_create_staff_notification', array( $this, 'trigger' ) );
...
return new MVR_Email_Staff_Assigned();
